  ...
  return (
    <main style={{ display: "grid", gridTemplateColumns: "2fr 4fr 5fr" }}>
      ...
      <div>
        {selectedJob && (
          <div>
            <h3 className="title"> Job Title :  { data?.filter(x=> x.id === selectedJob )[0].title  } </h3>
            <h3 className="Deadline"> Deadline : { data?.filter(x=> x.id === selectedJob )[0].deadline  } </h3>
            <h3 className="Deadline"> Views : { data?.filter(x=> x.id === selectedJob )[0].views  } </h3>
